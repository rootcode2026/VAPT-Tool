...
function ScansPage() {
  ...
  async function loadTargets() {
    try {
      ...
      const matchedTarget = list.find(
        (target) => target.id === queryTargetId
      );
      const projectTargets = queryProjectId
        ? list.filter(
            (target) => target.project_id === queryProjectId
          )
        : list;

      if (matchedTarget) {
        setSelectedTarget(matchedTarget.id);
            ...
            <div>
              ...
              >
                ...
                {(queryProjectId
                  ? targets.filter(
                      (target) =>
                        target.project_id === queryProjectId
                    )
                  : targets
                ).map((target) => (
                  <option
