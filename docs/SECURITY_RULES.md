...
- **Privacy:** no `stdout`/`stderr`/`source`/`secrets`/`tokens`/`cookies` in change events/run records/audit (`sanitize_change_metadata`, bounded evidence), monitoring scope capped 20 targets/runs per run, no external telemetry; scheduler tick returns counts only (no target values), `Scan.scan_metadata` holds IDs/profile/scanner keys/versions only.
- **Scheduler isolation (D1):** worker-side beat (`security_scheduler`, 60s floor, no sub-minute schedules), transactional claim (`FOR UPDATE` on Postgres) + overlap DEFER prevents duplicate runs/scans; scheduler never launches Docker (dispatches `execute_scan` only, verified by `test_scheduler_module_never_touches_docker`); pause clears `next_run_at` and blocks new claims without touching in-flight scans; capacity/buffer/health/version governance stays inside `execute_scan` (C9–C11 untouched).
- **Change detection (D2):** observations are project-scoped via the run's own project/scans (never cross-project); baseline snapshot is per-config and advances only on completed runs (failed/partial/empty can never poison it); absence events require a completed observation so failed scanners cannot fabricate removals; event identity is deterministic (`event_key` UNIQUE + `change_status` CAS) so concurrent/repeated processing cannot duplicate; states are bounded summaries with secret-adjacent keys dropped (no raw output/bodies/code/credentials in events or audit); new tables stay outside the RLS policy set (app-layer `require_project_access`, same posture as D1 tables); read API enforces project membership for viewer/analyst/admin alike with cross-project 404; no per-event audit noise.
- **Alerting (D3):** alerts derive ownership server-side from project/org (never client IDs); list/detail/ack/resolve verify `alert.project_id` against the path project (unknown/cross-project 404, no existence leak); lifecycle requires analyst+ (viewer read-only 403), policy requires project_admin+; acknowledge/resolve/policy audited with bounded sanitized metadata; dedup identity is deterministic per project+type+object so retries/concurrent processing cannot duplicate or cross tenants; resolutions only from D2 retraction events (never scanner failure); alert states/summaries bounded with secret-adjacent material dropped; alerts table outside RLS policy set (app-layer enforcement, documented posture).
- **SOC dashboard (D4):** strictly project-scoped aggregates (every query filters the path project; findings joined via targets; no org/global rollups); read-only for all project members, no new permission, no audit on reads; window params validated against 24h/7d/30d (400 otherwise); bounded lists (5/5/10/5) and bounded windows prevent resource exhaustion; response carries IDs/titles/counts only (no evidence/descriptions/raw metadata); failed runs surfaced as monitoring health, never as exposure; no caching layer (no cross-tenant cache risk).
- **Compliance evidence (D5):** evaluation is read-only over project-scoped findings/scans/assets/runs (joins via targets; catalog rows are global static knowledge, evidence rows always project-filtered); all members read, no write endpoints, no audit on reads; control/evidence IDs validated per project (unknown 404, cross-project 404, no existence leak); unknown frameworks rejected (400); evidence payloads carry bounded summaries only (no evidence/description/remediation bodies, no secrets); no RLS or permission changes; explicit readiness-only language (never certified/compliant).
- **Reporting (D6):** report ownership derived server-side (path project or org membership; never client IDs); detail/download verify project ownership (unknown/cross-project 404, no existence leak); generation requires analyst+ (viewer read/download only); download streams stored snapshots after authorization (never filesystem paths, never PDF bytes in list APIs); periods validated (explicit range, max 365d, 400 otherwise); detail lists capped (50/25) with truncation notes; artifacts immutable (regeneration creates new reports); finding titles included (consistent with finding APIs) but no evidence/remediation bodies beyond existing 200/300-char caps, no secrets/bodies/code; audit carries IDs/type/period only; no new tables/permissions/RLS changes.
...
- **Data:** explicit schemas only, SLA hours 1-8760, RA reason/expires required + justification on approve, remediation title/notes bounded 2000, retest evidence bounded sanitized, tags/reasons capped, audit `[REDACTED]`, no `password/JWT/token/cookie/stdout/source`.
