...
- **Full 23-section DOCX generation:** **NOT YET IMPLEMENTED.** Current `export_pdf` generates deterministic sections `Report Information, Executive Summary, Key Metrics, Key Findings, Limitations, Control Readiness` from snapshot — not the full DOCX layout (cover, TOC, detailed finding libraries, appendices, positive observations, remediation tracker, retest summary). **Limitation accurately documented:** customer report template requires mapping the 23 sections to persisted data; current PDF is infrastructure-complete but template-complete DOCX requires additional section mapping.
- **DOCX file:** `VAPT_Final_Report (1).docx` is **not committed** to repo (provided externally, confidential). Reference retained as authoritative; do not commit the binary.
...
Implement DOCX section mapping in `report_service.py` to fully populate the 23 sections without inventing data (use `collect_report_snapshot` + new section builders). Keep `GET /reports/{id}/download/pdf` as delivery mechanism; add `docx` format when template mapping complete.
