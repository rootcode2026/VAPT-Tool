...
- **Provider abstraction:** `backend/app/services/ai_provider.py` — `AIProvider` ABC (`generate(prompt, context, max_tokens) -> {provider, model, output, input_tokens, output_tokens}`), `MockAIProvider` (deterministic, bounded, mock-analyst, cite `[FINDING:id]`/`[ASSET:id]`, confidence low/medium, no external calls, timeout 5s guard), `OpenAIProvider` (OpenAI-compatible `/chat/completions`, `AI_BASE_URL`, `AI_API_KEY` never committed, `AI_TIMEOUT=30`, `AI_MAX_TOKENS` 1000, `temperature` 0.2, system/USER trust boundary messages), `NVIDIAKimiProvider` (Kimi K2 via `NVIDIA_API_KEY`/`NVIDIA_API_BASE_URL`/`NVIDIA_MODEL` — OpenAI-compatible `POST {base}/chat/completions` via `httpx` (fallback `requests`), server Bearer auth, enforce timeout `AI_TIMEOUT`, token bounds `AI_MAX_TOKENS`, safe categories 401/429/5xx/timeout/malformed/empty, never logs key, aliases `nvidia`/`kimi`/`kimi-k2`), `AIProviderRegistry` (`mock`/`openai`/`nvidia`/`kimi`/`kimi-k2`/`local`), `get_ai_provider()` respects `AI_ENABLED`+`AI_PROVIDER`. Replaceable without app rewrite; flow `Frontend → FastAPI ai route → ai_service → AIProvider → NVIDIAKimiProvider → NVIDIA Kimi K2`.
...
- **SOC dashboard (D4):** read-only aggregation over existing tables — `GET /api/v1/projects/{id}/dashboard/summary` (`api/routes/project_dashboard.py`, `require_project_access`, all project members, no new permission, no audit on reads) returning risk (AVG of recent scan-level scores from `RiskAssessmentEngine` output, same A/B/C/D cuts), finding/alert/asset/change aggregates plus bounded top/recent lists (top 5 findings/alerts, 10 changes, 5 runs; 24h/7d/30d windows); no new tables, no new risk algorithm, no caching; `/(app)/projects/[project_id]/dashboard`
page (risk/attention/changes/surface/health sections, skeleton/error/empty states, traceability links to existing detail pages), linked from projects list + attack-surface header. No SIEM/SOAR/notifications/compliance/AI.
- **Compliance evidence (D5):** versioned `vapt_control_readiness` 1.0 catalog (24 controls; rules in code, display rows seeded idempotently; existing demo frameworks untouched) + deterministic on-read evaluation (`services/control_evaluation.py`: explicit scanner→control matchers, PASS/PARTIAL/FAIL/NOT_ASSESSED/NOT_APPLICABLE, confidence HIGH/MEDIUM/LOW, freshness FRESH/STALE/UNKNOWN, accepted-risk yields PARTIAL at most, coverage = (PASS+PARTIAL)/(PASS+PARTIAL+FAIL+NOT_ASSESSED)); project APIs `GET /projects/{id}/compliance/{summary,controls,controls/{control_id},controls/{control_id}/evidence}` (`require_project_access`, all members read, no writes, no audit on reads); `/(app)/projects/[project_id]/compliance` page (coverage cards, filterable control table, evidence detail), linked from projects list. No new tables, no RLS changes, no reports/certification/AI.
- **Reporting (D6):** extends the existing reports subsystem (`models/report.py`, `services/report_service.py`, `api/routes/reports.py`) — no new tables, no new worker/queue. `collect_report_snapshot` assembles a bounded deterministic snapshot (period-scoped findings/scans/changes/alerts/runs, point-in-time assets, D5 control summary, scanner versions, methodology, data-driven limitations); `parse_report_period` enforces explicit periods (default 30d, max 365d, aware datetimes); deterministic executive summary from snapshot numbers only; `export_pdf` renders real sections via reportlab when present, else a valid dependency-free minimal PDF (correct xref); artifacts immutable (regenerated from stored snapshot); sync generation (bounded data, documented); routes accept `start_date/end_date` + `include_controls/include_low_findings`; `/(app)/reports` form gains period inputs, detail page uses authenticated blob downloads. Reports summarize evidence — never certification, legal advice, or audit.
